...
Referencia: Wingerchuk DM et al. International consensus diagnostic criteria for neuromyelitis optica spectrum disorders. Neurology. 2015;85:177-189.', 1, undefined)
...
N-MOmentum fue publicado formalmente como un ensayo fase 2/3, no como fase III pura. Referencia: Cree BAC et al. Lancet. 2019;394:1352-1363.', 1, undefined)
on conflict (id) do update set
  option_ids = excluded.option_ids,
  correct_option_id = excluded.correct_option_id;
...
Referencia: Montalban X et al. Diagnosis of multiple sclerosis: 2024 revisions of the McDonald criteria. Lancet Neurol. 2025.', 1, undefined)
on conflict (id) do update set
  option_ids = excluded.option_ids,
  correct_option_id = excluded.correct_option_id;
...
Referencia: Banwell B et al. Diagnosis of MOG antibody-associated disease: International MOGAD Panel proposed criteria. Lancet Neurol. 2023;22:268-282.', 1, undefined)
on conflict (id) do update set
  option_ids = excluded.option_ids,
  correct_option_id = excluded.correct_option_id;
...
Referencia: Bjornevik K et al. Longitudinal analysis reveals high prevalence of Epstein-Barr virus associated with multiple sclerosis. Science. 2022;375:296-301. doi:10.1126/science.abj8222.', 1, undefined)
on conflict (id) do update set
  option_ids = excluded.option_ids,
  correct_option_id = excluded.correct_option_id;
...
La estabilidad no se define sólo por el paso del tiempo: deben revisarse las recaídas, la recuperación, la RM, el tratamiento utilizado y su eventual período de lavado o riesgo de rebote. La edad materna y la reserva ovárica también condicionan la decisión, por lo que no debe postergarse el tratamiento ni la planificación sin una evaluación individual.', 1, undefined)
on conflict (id) do update set
  option_ids = excluded.option_ids,
  correct_option_id = excluded.correct_option_id;
...
Referencia: Kappos L et al. Inclusion of brain volume loss in a revised measure of no evidence of disease activity (NEDA-4). Mult Scler. 2016;22:1297-1305.', 1, undefined)
on conflict (id) do update set
  option_ids = excluded.option_ids,
  correct_option_id = excluded.correct_option_id;
...
Referencia: Kalb R et al. Recommendations for cognitive screening and management in multiple sclerosis care. Mult Scler. 2018;24:1665-1680.', 1, undefined)
on conflict (id) do update set
  option_ids = excluded.option_ids,
  correct_option_id = excluded.correct_option_id;
